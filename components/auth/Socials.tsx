...
const Socials =({
  ...
  const handleLoginWith = ({provider} : {provider: GithubAuthProvider | GoogleAuthProvider}) => {
  ...
  return (
    <div className='flex flex-col justify-center gap-4 w-full items-center'>
      ...
      <Button
        ...
        onClick={() => handleLoginWith({ provider: gitHubProvider })}
        disabled={loading}
      >
      ...
      <Button
        variant="outline"
        className="w-[50%] cursor-pointer"
        onClick={() => handleLoginWith({ provider: googleProvider })}
        disabled={loading}
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path
